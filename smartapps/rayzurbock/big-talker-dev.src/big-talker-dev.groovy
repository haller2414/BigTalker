...
def processMotionEvent(index, evt){
    ...
    if (evt.value == "active") {
        ...
        if (!(state?.TalkPhrase == null)) {Talk(state.TalkPhrase, state.speechDevice, resume, evt)} else {LOGDEBUG("Not configured to speak for this event")}
    }
		...
		if (!(settings.speechDeviceDefault == null) || !(customSpeechDevice == null)) {
			...
			if (state.ableToTalk){
				...
				currentSpeechDevices.each(){
					...
					if (desiredVolume > -1){ 
                    	LOGTRACE("TALK(${evt.name})|mP| ${it.displayName} | Volume: ${currentVolume}, Desired Volume: ${desiredVolume}")
                    } else {
	                    if (!(currentVolume >= minimumVolume)) {
    	                	LOGTRACE("TALK(${evt.name})|mP| ${it.displayName} | Volume: ${currentVolume}, Minimum Volume: ${minimumVolume}; adjusting.")
        	            } else {
            	        	LOGTRACE("TALK(${evt.name})|mP| ${it.displayName} | Volume: ${currentVolume}, Minimum Volume: ${minimumVolume}; acceptable.")
                	    }
                    }
